(function ($) {
  ...
  function mainNav() {
    ...
    // Side Nav
    $(".desk-menu_btn").on("click", function () {
      $(".menu_desktop").addClass("active");
    });
     $(".mob-menu_btn").on("click", function () {
...
const cards = document.querySelector(".sw-cards");
const line = document.querySelector(".sw-line-progress");
const steps = document.querySelectorAll(".sw-step");

function updateTimeline() {
  const maxScroll = cards.scrollHeight - cards.clientHeight;
  const progress = maxScroll > 0 ? cards.scrollTop / maxScroll : 0;

  // ALWAYS vertical fill (desktop + mobile)
  line.style.height = `${progress * 100}%`;
  line.style.width = "2px";

  steps.forEach((step, index) => {
    const point = index / (steps.length - 1);
    step.classList.toggle("active", progress >= point);
  });
}

cards.addEventListener("scroll", updateTimeline);
...
document.querySelectorAll(".menu-btn").forEach(btn => {
  ...
  // sticky card section start

  // const section = document.getElementById("stackSection");
const cardStyle = document.querySelectorAll(".stack-card");
...
function handleScroll() {
  ...
  cardStyle.forEach((card, index) => {
    const start = index * 0.25;
    const end = 1;

    let progress = (scrollProgress - start) / (end - start);
    progress = clamp(progress, 0, 1);

    const targetScale = Math.max(
      0.5,
      1 - (cardStyle.length - index - 1) * 0.1
    );

    const scale = 1 + (targetScale - 1) * progress;
    const translateY = index * 20 + 250;
